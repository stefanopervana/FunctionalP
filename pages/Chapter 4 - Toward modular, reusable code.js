...
Even if you have no experience with Unix programming, you can clearly see that this code involves a sequence of steps that transforms words from uppercase to lowercase, removes duplicates, and sorts the remainder. The pipe operator ( | in this case) connects these commands. It’s remarkable that by having clear contracts describing the inputs and outputs, small programs can be glued together to solve complex tasks. If you imagine having to write this program in traditional imperative JavaScript, a few loops, string comparisons, and perhaps a few conditional statements and global variables keeping track of everything come to mind. This probably isn’t very modular, per se. In programming, we like to solve problems by breaking them into smaller pieces and reconstructing those pieces to form a solution.
...
Creating modular code isn’t an easy task. We’ll look at important functional techniques like partial evaluation and composition, with the aid of a functional framework called Ramdajs, to bring code to the right level of abstraction in order to express solutions in a point-free manner via declarative function pipelines.
4.1 Method chains vs. function pipelines Chapter 3 left off with method chains used to connect a series of functions together, revealing a style of functional programming much different from any other development style. But there’s another approach for connecting functions, called pipelining.
When studying functions, it’s useful to describe them in terms of their inputs and outputs. The notation used in Haskell, for example, is popular in the functional community, and you’ll see it used in many places (see figure 4.1).
 
<p className="pag">Page 86</p>
 
Remember that, in functional programming, a function is a mathematical mapping between inputs and output types, as shown in figure 4.2. For instance, a simple function like isEmpty that takes a string and returns a Boolean can be expressed in this notation as
...
At a high level, you can visualize a simple sequence of array methods as shown in figure 4.3. It would be best to break the chain (so to speak) and have a lot more freedom to arrange a sequence of independent functions; you can achieve this with function pipelines.
4.1.2	Arranging functions in a pipeline
Functional programming removes the limitations present in method chaining and provides the flexibility to combine any set of functions, no matter where they come from. A pipeline is a directional sequence of functions loosely arranged so that the output of one is input into the next. Figure 4.4 illustrates this abstractly by connecting functions that work with different types of objects.
...
NOTE Statically typed languages have the advantage of using type systems to alert you about potential problems without having to run your code. Type systems are an important topic in functional programming but aren’t covered in this book.
...
<li className="litag">	Avoid creating ad hoc types—Tuples can relate values that may have no relationship at all to each other. So defming and instantiating new types solely for grouping data together makes your model unnecessarily convoluted.</li>
...
The Tuple object in listing 4.2 is an immutable, fixed-length structure used to hold a heterogeneous set of n typed values that can be used for inter-function communication. For instance, you can use it to build quick value objects, such as Status:
...
The occurrence of 2-tuples is so frequent in software that it’s worth making them firstclass objects. When combined with JavaScript ES6 support for destructured assignment, you can map tuple values to variables in a clean manner. Using tuples, the following code creates an object called StringPair.
Listing 4.4 StringPair type
Tuples are one way to reduce a function’s arity, but there’s a better alternative for cases in which tuples aren’t sufficient. Let’s spice things up a bit by introducing function currying, which not only abstracts arity but also encourages modularity and reusability.
...
Like Lodash, Ramda.js provides lots of useful functions to connect functional programs and also enables a pure functional style of coding. The reason for using it is that its parameters are conveniently arranged to facilitate currying, partial application, and composition, which I'll cover later in this chapter. For more details about setting up Ramda, see the appendix.
...
For simple tasks, curry2 is adequate; but as you start building more-complex functionality, you’ll need to handle any number of arguments automatically. Normally, I’d show you the function internals, but curry is a particularly long and convoluted function to explain, so I’ll spare you the headache and move into a more useful discussion (you can find curry and its flavors—curryRight, curryN, and so on—implemented in both Lodash and Ramda).
You can use R.curry to simulate the automatic currying mechanism in pure functional languages that works on any number of arguments. You can imagine automatic currying as artificially creating nested function scopes corresponding to the number of arguments declared. This example curries fullname:
...
Now, findStudent can be passed to other modules without the caller knowing the concrete implementation (this will be important in chapter 6 for unit testing to mock interaction with the object store). In matters of reuse, currying also allows you to create a family of function templates.
4.3.2	Implementing reusable function templates
Suppose you need to configure different logging functions to handle different states in your application, such as errors, warnings, debug, and so on. Function templates define a family of related functions based on the number of arguments that are curried at the moment of creation. This example will use the popular library Log4js, a logging framework for JavaScript that is far superior to the typical console.log. You can find installation information in the appendix. Here’s the basic setup:
...
In addition to gaining lots of reusability in your code, as I mentioned, the principal motivation behind currying is to convert multiargument functions into unary functions. Alternatives to currying are partial function application and parameter binding, which are moderately supported by the JavaScript language, to produce functions of smaller arity that also work well when plugged into function pipelines.
...
<li className="litag">	Currying generates nested unary functions at each partial invocation. Internally, the final result is generated from the step-wise composition of these unary functions. Also, variations of curry allow you to partially evaluate a number of arguments; therefore, it gives you complete control over when and how evaluation takes place.</li>
<li className="litag">	Partial application binds (assigns) a function’s arguments to predefined values and generates a new function of fewer arguments. The resulting function contains the fixed parameters in its closure and is completely evaluated on the subsequent call.</li>
...
Both partial application and currying are useful. Currying is the most widely used technique to create function wrappers that abstract a function’s behavior, either to preset its arguments or to partially evaluate them. This is beneficial because pure functions with fewer arguments are easier to work with than functions with many arguments. Either approach facilitates supplying the proper arguments so that functions don’t have to blatantly access objects outside of their scope, while reducing them to unary functions. Isolating the logic of obtaining this necessary data makes functions more reusable; and, more important, it simplifies their composition.
...
The idea of composition is intuitive and certainly not unique to functional programming. Consider how HTML widgets are laid out on a page. Complex widgets are built from the combination of simple ones, which in turn can form part of even bigger widgets. For instance, combining three input text boxes with an empty container produces a simple student form, as shown in figure 4.7.
...
This is more or less how lists are constructed in languages like ML and Haskell. On the other hand, complex objects with high degrees of coupling to other external objects don’t have clear rules for composition and can be extremely hard to work with. Functional composition can have a similar fate when side effects and mutations are present. Now, let’s dive into the composition of functions.
...
Arguably, this code is easy to read, and its meaning easily derived by glancing at the function’s constituent parts. The interesting quality of this program is that evaluation never takes place until countWords is run; in other words, the functions passed by name (explode and count) are dormant within the composition. The result of composition is another function that waits to be called with its respective argument: the argument to countWords. This is the beauty of function composition: separating a function ’s description from its evaluation.
...
This concept isn’t limited to functions; entire programs can be built from the combination of other side effect-free, pure programs or modules. (Based on the earlier definition of a function, used throughout this book, I’ll use the terms function, program, and module loosely to refer to any executable unit with inputs and output.)
...
If you like this better, feel free to use it. In the next chapter, you’ll learn that this mechanism of chaining methods is prevalent in functional algebraic data types called monads. Personally, I recommend sticking to the more functional form, because it’s much more succinct and flexible and works better in conjunction with functional libraries.
4.5.3 Composition with functional libraries
One of the benefits of working with a functional library such as Ramda is that all functions have been properly configured with currying in mind, making them versatile when composed into function pipelines. Let’s look at another example. Here’s a list of students with their respective grades in a class:
<pre>const students = ['Rosser', 'Turing', 'Kleene', 'Church'];
const grades =	[80, 100,	90,	99];</pre>
Suppose you need to find the student with the highest grade in the class. You learned in chapter 3 that working with collections of data is one of the cornerstones of functional programming. The code in listing 4.9 is made up of the composition of several curried functions, each in charge of transforming this data in a particular way:
...
In addition, something you’ll soon realize and begin to love about functional composition is how you’re naturally drawn to expressing the entire solution succinctly in one or two lines. Because you’re forced to create functions that map to the different stages in your algorithm, you begin to build an ontology with which you can stitch together expressions that describe parts of your solution, allowing team members to more quickly understand your code. The following listing is similar to an exercise from chapter 3.
...
Although this instance of the program is easier to read, there’s no added reusability because these functions are specific to the particular task at hand. Rather, I recommend getting acquainted with the functional vocabulary of head, pluck, zip, and others, so that, through practice, you gain comprehensive knowledge of your functional framework of choice. It will make the transition to other frameworks or other functional languages easier, because they all use many of the same naming conventions. This will quickly pay dividends in your productivity.
...
Let’s continue improving this program by using curry to partially evaluate the invariable parameters of each function. You’ll also add code to sanitize the input parameter and refactor the HTML operations with more fine-grained functions. Finally, you’ll make the find operation more functional by dislodging it from the object store.
...
If the thought of composing functions in this naturally reversed flow feels odd to you, or you visualize your programs as a left-associative sequence, you can use Ramda’s mirror function to compose, called pipe, which achieves the same results:
<pre>R.pipe(
trim,
normalize,
findStudent,
...
As evidence of how important this is, F# provides built-in support for this using its pipe-forward operator |>. In JavaScript, we don’t have this luxury, but we can safely rely on functional libraries to do the job effectively Note from both R.pipe and R.compose that you’re creating new functions without having to explicitly declare any of their formal arguments, as you’d normally have to. Functional composition encourages this writing style, which goes by the name of point-free coding.
 

 
<p className="pag">Page 111</p>
4.5.5	Introducing point-free programming
If you look closer at the function in listing 4.10, you can see that it doesn’t show the parameters of any of its constituent functions, as would a traditional function declaration. Here it is again:
...
<li className="litag">	Supplying data to higher-order functions that expect it when evaluating a function argument, as you did earlier when writing point-free code (listing 4.12).</li>
...
To understand what’s happening, think of this code emulating a simple if-else statement equivalent to the imperative conditional logici:
<pre>
...
Some people view composition as restrictive, but you can see for yourself that it’s quite the opposite: combinators unlock freedom and facilitate point-free programming. Because combinators are pure, they can be composed into other combinators, providing an infinite number of alternatives to express and reduce the complexity of writing any type of application. You’ll see them used again in the following chapters.
...
<li className="litag">	Currying and partial evaluation can be used to reduce the arity of pure functions by partially evaluating a subset of a function’s arguments and transforming them into unary functions.</li>
