...
In this chapter, I’il introduce you to a few useful and practical operations like map, reduce, and filter that allow you to traverse and transform data structures in a sequential manner. These operations are so important that virtually all functional programs use them in one way or another. They also facilitate removing manual loops from your code, because most loops are just specific cases handled by these functions.
...
This code follows the functional doctrine that all parameters should be explicitly defined in the function declaration; it has no side effects and doesn’t mutate the original object. Arguably, writing this function inside out isn’t as fluent as the methodchaining approach. It’s also much harder to read, because you need to start peeling off the wrapped functions to understand what’s truly happening.
...
Recali from chapter 1 that functional programs are made up of simple functions that in themselves don’t accomplish much but, when put together, can solve complex tasks. In this section, you’ll learn a way to build entire an program by linking a set of functions.
...
On the other hand, the functional implementation merely connects black-box componente together and cedes the responsibility to these well-established and tested APIs, as shown in the following listing. Notice how the cascade arrangement of function calls makes this code easier to read.
...
I’d much rather write and read programs that look like listing 3.7, wouldn’t you? Notjust because of the sheer reduction in the amount of code, but also due to its simple and clear structure.
...
In the object-oriented world, it's also another way to reuse code without having to use inheritance or to simulate multiple inheritance in languages that don't support it (JavaScript being one of them). I won't cover mixins in this book, but they can be powerful when used correctly. If you want to learn more about mixins, I suggest reading https://javascriptweblog.wordpress.com/2011/05/31/a-fresh-look-at-javascriptmixins/.
...
<li className="litag">	Functional programming’s declarative style creates code that’s easier to reason about.</li>
<li className="litag">	Mapping high-level abstractions to a SQL vocabulary reveals a deeper understanding of your data.</li>
<li className="litag">	Recursion solves self-similar problems and is required to parse through recursively defined data structures.</li>
