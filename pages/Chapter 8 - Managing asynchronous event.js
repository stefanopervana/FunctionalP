...
<li className="litag">	Applying reactive programming to tackle event- driven code</li>
...
Before you read this book, this code would’ve looked acceptable to you; but to a func- tional programmer such as yourself, it looks messy and tangled (later, I’ll show you a complete functional version of this code). The same effect occurs when handling events. Listing 8.3 interleaves AJAX calls with user-input handling. It listens for clicks and mouse events, fetches multiple pieces of data from the server, and renders the data on the DOM.
...
When a program begins taking this form, you find yourself relying on spacing and syntac- tic organization, such as grouping statements just to improve readability. But this is just putting lipstick on a pig. Let’s see how thinking functionally can help in this situation.
...
Using continuations also fixes a problem in listing 8.2 that occurs when interlac- ing synchronous and asynchronous behavior. The problematic section of code is the
...
The previous code example is defmitely an improvement over the imperative asyn- chronous programs you saw at the beginning of the chapter, but it’s far from being functional. As with any functional program, you also seek other qualities like these:
...
<li className="litag">	Abstracting the notion of temporal coupling so that you don’t need to be con- cerned with it</li>
...
Now, instead of continuation passing, you’ll refactor listing 8.2 with a superior solu- tion based on promises. Here’s the code from listing 8.2 once more:
getJSON('/students',
function (students) {
...
<li className="litag">	Take advantage of Ramda’s curried functions to create succinct data-transfor- mation steps like sorting, filtering, and mapping.</li>
...
But monads aren’t effective only for forming method chains. As you learned in previ- ous chapters, they’re also effective when used with composition.
...
One of ES6’s most powerful features is the ability of functions to cooperate with others by pausing to provide data without necessarily running to completion. This brings many (possibly infinite) opportunities for functions to become vehicles for lazily pro- ducing data instead of having to process massive data structures all at once.
...
You can easily model the data included in the branches of this tree using simple gen- erators like so (I’ll show the printed result of running this program later):
...
Generators are closely tied to another ES6 artifact called iterators, which is the reason you can loop over generators like any other data structure (such as arrays). Behind the scenes, a generator function returns a Generator object that conforms to the iter- ator protocol; this means it implements a method called next() that returns a value resulting from using the yield keyword. This object has the following properties:
<li className="litag">	done—Has the value true if the iterator is passed the end of the sequence. Oth- erwise, a value of false means the iterator was able to produce another value in the sequence.</li>
...
An observable is any data object that you can subscribe to. Applications can subscribe to asynchronous events emitted from reading a file, a web service call, querying a data- base, pushing system notifications, handling user input, traversing a collection of ele- ments, or even parsing a simple string. Reactive programming unifies all of these data providers into a single concept called an observable stream, using the Rx.Observable
...
The Rx.Observable object unites the world of functional and reactive programming. It implements the equivalent of the minimal monadic interface you learned about in chapter 5 (map, of, and join) as well as many methods specific to stream manipula- tion. Here’s a quick example:
...
Suggested reading
Reactive programming has been on the rise since 2013, so a sizeable amount of con- tent is available related to it and FRP. My goal isn't to teach you reactive program- ming, but to demonstrate that reactive programming is really functional programming applied to asynchronous and event-based problems.
...
<li className="litag">	Generators take another approach to asynchronous code by providing pro- gramming artifacts, backed by lazy iterators, that allow you to yield for data to be available.</li>
<li className="litag">	Functional reactive programming raises the level of abstraction of your pro- grams so that you can focus on treating events as logically independent units. This lets you focus on your task at hand instead of coping with complex imple- mentation details.</li>
