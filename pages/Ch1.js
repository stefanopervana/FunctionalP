...
<p className="it">OO hace al código comprensible mediante la encapsulación de las piezas móviles.</p>

<p className="p">OO makes code understandable by encapsulating moving parts.</p>
<p className="it">FP hace al código entendible minimizando las piezas móviles.</p>

<p className="p">FP makes code understandable by minimizing moving parts.</p>


<p className="p">—Michael Feathers (Twitter)</p>
<p className="it">Si usted está leyendo este libro, es probable que usted es un desarrollador de JavaScript con un conocimiento del trabajo de diseño orientado a objetos o estructurado, y tiene curiosidad acerca de la programación funcional. Tal vez usted ha tratado de aprenderla antes y no ha sido capaz de aplicarla con éxito en el trabajo o en sus proyectos personales. En cualquier caso, su principal objetivo es mejorar sus habilidades de desarrollo y mejorar la calidad de su código. Este libro puede ayudar a lograr eso.</p>

<p className="p">If you’re reading this book, chances are you’re a JavaScript developer with a working knowledge of object-oriented or structured design, and you’re curious about functional programming. Perhaps you’ve tried to learn it before and haven’t been able to apply it successfully at work or on your personal projects. In either case, your main goal is to advance your development skills and improve the quality of your code. This book can help you accomplish that.</p>
...
<p className="it">El diseño orientado a objetos ayuda a resolver parte del problema; sino fuera porque JavaScript es un lenguaje dinámico con una gran cantidad de estado compartido, no pasa mucho tiempo antes de acumular suficiente complejidad para hacer nuestro código difícil de manejar y difícil de mantener. El diseño orientado a objetos desde luego mueve la aguja en la dirección correcta, pero necesitamos más. Tal vez usted ha oído sobre la programación reactiva en los últimos años. Este paradigma de programación facilita el trabajo con los flujos de datos y la propagación del cambio. En JavaScript, esto es muy importante cuando se trata de un código asíncrono o basado en eventos. En general, lo que necesitamos es un paradigma de programación que nos anima a pensar cuidadosamente acerca de nuestros datos y las funciones que interactúan con él. Al pensar en el diseño de una aplicación, hágase las siguientes preguntas en términos de estos principios de diseño:</p>
...
<li className="littag"> Extensibilidad - 
¿Refactorizo constantemente mi código para admitir funcionalidades adicionales?</li>
<li className="litag">	Extensibility — Do I constantly refactor my code to support additional functionality?</li>
<li className="littag"> Fácil de modularizar - Si cambio un archivo, se ve afectado otro archivo?</li>
...
<li className="littag"> La capacidad de prueba - ¿Me esfuerzo para probar mis funciones?</li>
<li className="litag">	Testability — Do I struggle to unit test my functions?</li>
<li className="littag">Fácil de razonar sobre si - ¿Es mi código desestructurado y difícil de seguir?</li>
<li className="litag">	Easy to reason about—Is my code unstructured and hard to follow?</li>

<p className="it">Si su respuesta es “Sí” o “No sé” a cualquiera de estas preguntas, entonces usted ha recogido el libro correcto como guía en el camino hacia la productividad. La programación funcional (FP) es el paradigma de programación que necesita. A pesar de que se basa en conceptos simples, FP requiere un cambio en la manera de pensar acerca de los problemas. FP no es una nueva herramienta o una API, sino un enfoque diferente para la resolución de problemas que se convertirá en intuitivo una vez que conosca los principios básicos.</p>

<p className="p">If you answer “Yes” or “I don’t know” to any of these questions, then you’ve picked up the right book as a guide on the path to productivity. Functional programming (FP) is the programming paradigm you need. Although it’s based on simple concepts, FP requires a shift in the way you think about problems. FP isn’t a new tool or an API, but a different approach to problem solving that will become intuitive once you understand the basic principles.</p>
<p className="it">En este capítulo, defino lo que la programación funcional es y cómo y por qué digo que es útil e importante. Introduzco los principios básicos de funciones inmutabilidad y puros y hablo acerca de las técnicas de FP y cómo esas técnicas afectan a su enfoque para el diseño de programas. Estas técnicas permiten elegir fácilmente la programación reactiva y lo utilizan para resolver tareas complejas de JavaScript. Pero antes de que podamos entrar en todo esto, es necesario saber por qué pensar funcionalmente es importante y cómo se puede ayudar a hacer frente a las complejidades ofJavaScript programas.</p>
...
<p className="it">El aprendizaje de la programación funcional nunca ha sido tan importante como lo es hoy. La comunidad de desarrollo y de las principales compañías de software están empezando a darse cuenta de los beneficios del uso de técnicas de FP para hacer funcionar sus aplicaciones de negocio. Hoy en día, la mayoría de los principales lenguajes de programación (Scala, Java 8, F #, Python, JavaScript y muchos más) proporcionan apoyo funcional, ya sea nativo o basado en la API. Por lo tanto, las habilidades de PF están en alta demanda ahora y seguirán siendolo en los años venideros.</p>
...
<p className="it">Normalmente, los libros de PF comienzan con el cálculo de los números de Fibonacci, pero prefiero empezar con un simple programa JavaScript que muestra el texto en una página HTML. ¿Qué mejor texto para imprimir que el buen “Hello World”:</p>
...
<p className="it">¿Por qué la solución funcional mira de esta manera? Me gusta pensar que es básicamente la parametrización de su código para que pueda cambiar fácilmente en un mannerno invasiva como el ajuste de las condiciones iniciales de un algoritmo. Con este fundamento puesto, ahora se puede aumentar fácilmente printMessage para repetir el mensaje dos veces, utilizar un encabezado h2, y escribir en la consola, en lugar de la DOM, todo ello sin tener que volver a escribir cualquiera de la lógica interna.</p>
...
<p className="p">Functional programming falls under the umbrella of declarative programming paradigms: it’s a paradigm that expresses a set of operations without revealing how they’re implemented or how data flows through them. The more popular models used today, though, are imperative or procedural, and are supported in most structured and objectoriented languages like Java, C#, C++, and others. Imperative programming treats a computer program as merely a sequence of top-to-bottom statements that changes the state of the system in order to compute a result.</p>
...
<p className="it">El cambio a un enfoque funcional para hacer frente a esta misma tarea, sólo tiene que ser concerned con aplicar el comportamiento correcto en cada elemento y ceder el control de bucle a otras partes del sistema. Puede dejar que Array.map () hacer la mayor parte del trabajo pesado:</p>
<p className="p">Shifting to a functional approach to tackle this same task, you only need to be concerned with applying the right behavior at each element and cede control of looping to other parts of the system. You can let Array.map() do most of the heavy lifting:</p>
<p className="it">En comparación con el ejemplo anterior, se ve que este código le libera de la responsabilidad de gestionar adecuadamente un contador de bucle y el acceso índice de la matriz; en pocas palabras, cuanto más código que tiene, más lugares hay para los insectos que se produzca. Además, los bucles estándar no son artefactos reutilizables a menos que estén resumieron con las funciones. Y eso es precisamente lo que vamos a hacer. En el capítulo 3, demuestro cómo eliminar bucles manuales completamente de su código en favor de la primera clase, funciones de orden superior, como un mapa, reducir y filtro, que acepta funciones como parámetros para que su código es más reutilizable, extensible y declarativo. Esto es lo que hice con la función de ejecución mágico en los listados 1.1 y 1.2.</p>
...
<p className="p">Lambda expressions provide an enormous syntactical advantage over regular function notations because they reduce the structure of a function call down to the most important pieces. This ES6 lambda expression:</p>
...
<p className="it">La programación funcional se basa en la premisa de que construir programas inmutables sobre la base de los componentes básicos de las funciones puras. Una función pura tiene las cualidades siguientes:</p>
<p className="p">Functional programming is based on the premise that you build immutable programs based on the building blocks of pure functions. A pure function has the following qualities:</p>
...
<p className="p">Let’s analyze this code further. This function clearly exposes a few side effects that ripple beyond its scope:</p>
...
<li className="littag">Más importante, la interacción con los objetos HTML se trasladó a su propia función, aislando el puro a partir del comportamiento no-pura (impuro). Me explico ing curryy la gestión de partes puras e impuras en profundidad en el capítulo 4.</li>
<li className="litag">	More important, interaction with the HTML objects is moved into its own function, isolating the pure from the non-pure (impure) behavior. I explain currying and managing pure and impure parts in depth in chapter 4.</li>
...
<p className="it">Ahora esta función es estable y siempre devuelve la misma salida cuando se proporciona con la misma entrada. De lo contrario, el valor devuelto por la función está siendo influenciado por algún factor externos.</p>
<p className="p">Now this function is stable and always returns the same output when provided with the same input. Otherwise, the function’s return value is being influenced by some external factor.</p>
<p className="it">Buscamos esta calidad en funciones, ya que no sólo hace que el código más fácil de probar, sino que también nos permite razonar sobre programas enteros con mucha más facilidad. transparencia referencial o corrección ecuacional se heredan de matemáticas, sino que funciona en lenguaje de programación se comportan nada como funciones matemáticas; consiguiendo así perdería transparencia referencial es estrictamente en nosotros. Utilizando la función de ejecución mágica de nuevo, la figura 1.2 muestra cómo utilizar el imperativo frente a la versión funcional de incremento.</p>
<p className="p">We seek this quality in functions because it not only makes code easier to test, but also allows us to reason about entire programs much more easily. Referential transparency or equational correctness is inherited from math, but functions in programming languages behave nothing like mathematical functions; so achieving referential transparency is strictly on us. Using the magical run function again, figure 1.2 shows how to use the imperative versus the functional version of increment.</p>
...
<p className="it">Si las funciones [funcl, func2, func3, ...] son &#8203;&#8203;puros, se puede reescribir fácilmente este programa por inlining los valores producidos por ellos[vali, val2, val3, ...] sin alterar el resultado. Consideremos un ejemplo sencillo de calcular el grado promedio de un estudiante:</p>
<p className="p">If the functions [funcl, func2, func3, ...] are pure, you can easily rewrite this program by inlining the values produced by them—[vali, val2, val3, ...]—without altering the result. Consider a simple example of computing a student’s average grade:</p>
<pre><code>{`
var input =	[80,	90, 100];
...
<p className="it">A primera vista, este código parece perfectamente bien y es partidario del libre efecto. Se hace lo que se espera que haga-usted proporciona una matriz y devuelve la misma matriz ordenada en orden descendientes ING:</p>
<p className="p">At a glance, this code seems perfectly fine and side effect-free. It does what you’d expect it to do—you provide an array, and it returns the same array sorted in descending order:</p>
<pre><code>{`var arr =	[1,2,3,4,5,6,7,8,9];
sortDesc(arr); //->	[9,8,7,6,5,4,3,2,1]`}</code></pre>
<p className="it">Desafortunadamente, la función Array.sort es con estado y provoca el efecto secundario de la clasificación de la matriz en lugar-la referencia original se cambia. Este es un serio defecto en el lenguaje y que vamos a superar en los próximos capítulos.</p>
<p className="p">Unfortunately, the Array.sort function is stateful and causes the side effect of sorting the array in place—the original reference is changed. This is a serious flaw in the language and one that we’ll overcome in future chapters.</p>
<p className="it">Ahora que usted ha tenido una visión de los principios fundamentales de la programación funcional (declarativa, pura, e inmutable), puedo expresar lo que es más Suc cinctly: programación funcional se refiere a la evaluación de las funciones declarativa puros para crear programas inmutables por evitando los efectos secundarios observables externamente. No es un bocado después de todo. Sólo he arañado la superficie en términos de los beneficios prácticos de la escritura de aplicaciones funcionales, pero por ahora se está empezando a entender lo que significa pensar con esta mentalidad.</p>
<p className="p">Now that you’ve had a glimpse of the fundamental principles behind functional programming (declarative, pure, and immutable), I can express what it is more succinctly: functional programming refers to the declarative evaluation of pure functions to create immutable programs by avoiding externally observable side effects. Not such a mouthful after all. I’ve only scratched the surface in terms of the practical benefits of writing functional applications, but by now you’re beginning to understand what it means to think with this mindset.</p>
...
<p className="p">All along, I’ve been using run to combine functions to make up whole programs. It’s time to uncover this dark magic. In reality, run is an alias for one the most important techniques: composition. The composition of two functions is another function that results from taking the output of one and plugging it into the next. Assume that you have two functions f and g. Formally, this can be expressed as follows:</p>
<pre><code>{`
f • g = f(g(x))`}</code></pre>
...
<p className="p">In addition to map, you can import a repertoire of many higher-order functions into any JavaScript project through some powerful and optimized functional libraries. In chapters 3 and 4, I give a tour of many of these higher-order functions implemented in popular JavaScript functional toolkits like Lodash.js and Ramda.js; they overlap in many aspects, but each brings unique features that can facilitate assembling function chains.</p>
...
<p className="p">If you’ve written some JQuery code before, you’re probably familiar with this idiom. A chain is a sequential invocation of functions that share a common object return value (such as the S or jQuery object). Like composition, this idiom allows you to write terse and concise code, and it’s typically used a lot in functional as well as reactive programming JavaScript libraries (more on this later). To show this, let’s tackle a different problem. Suppose you’re tasked with writing a program that computes the average grade for studente who have enrolled in more than one class. Given this array of enrollment data:</p>
...
<p className="p">Now you can use Lodash to stitch together functions representing these steps, forming a functional chain, as shown in listing 1.5 (for a full explanation of what each of these functions does, you can visit the appendix for directions on where to fmd the proper documentation). A function chain is a lazy evaluated program, which means it defers its execution until needed. This benefits performance because you can avoid executing entire sequences of code that won’t be used anywhere else, saving precious CPU cycles. This effectively simulates the call-by-need behavior built into other functional languages.</p>
...
<p className="p">So far, you’ve seen how FP can help you create modular, testable, extensible applications. How well does it work when you need to interact with asynchronous or event-based data coming from user input, remote web requests, file systems, or persistent storage?</p>
<h2 className="hdos">1.3.3	Reacting to the complexity of asynchronous applications</h2>
<p className="it">Si recuerda la última vez que tuvo que obtener los datos remotas, manejar la entrada del usuario, o interactuar con el almacenamiento local, es probable que recuerdan la escritura secciones enteras de la lógica de negocio en secuencias anidadas de funciones de devolución de llamada. Este patrón de devolución de llamada rompe el flujo lineal de su código y se hace difícil de leer, porque está atestado de formas anidadas de la lógica éxitoy control de errores. Esto es todo a punto de cambiar.</p>
<p className="p">If you remember the last time you had to fetch remote data, handle user input, or interact with local storage, you probably recall writing entire sections of business logic into nested sequences of callback functions. This callback pattern breaks the linear flow of your code and becomes hard to read, because it’s cluttered with nested forms of successand error-handling logic. This is all about to change.</p>
<p className="pag">Page 20</p>
<p className="it">Como he dicho antes, el aprendizaje de la programación funcional, especialmente para los Ircops desaJavaScript, es muy importante hoy en día. Cuando la construcción de aplicaciones de gran tamaño, una gran parte de la atención se ha desplazado de los frameworks orientados a objetos como Backbone.js a los frameworks que favorezcan un paradigma de programación reactiva. frameworks web como AngularJS siguen siendo ampliamente utilizados hoy en día; pero los nuevos jugadores en el campo, tales como RxJS, abrazan el poder de FP para hacer frente a tareas muy difíciles.</p>
<p className="p">As I said earlier, learning functional programming, especially for JavaScript developers, is extremely important today. When building large applications, a lot of the focus has shifted from object-oriented frameworks like Backbone.js to frameworks that favor a reactive programming paradigm. Web frameworks like Angularjs are still widely used today; but new players in the field, such as RxJS, embrace the power of FP to tackle very challenging tasks.</p>
<p className="it">la programación reactiva es probablemente una de las más excitantes e interesantes aplicaciones de programación funcional. Se puede utilizar para reducir drásticamente la complejidad de código asíncrono y orientada a eventos que, como desarrolladores de JavaScript, se ocupa de todos los días en el cliente, así como el servidor.</p>
...
<p className="it">El principal beneficio de la adopción de un paradigma reactiva es que aumenta el nivel de abstracción de su código, lo que le permite centrarse en la lógica de negocio específico, mientras que olvidando sobre el código repetitivo ardua asociados con la apertura asincrónica y programas basados &#8203;&#8203;en eventos. Además, este paradigma emergente aprovecha al máximo dad abilde FP a la cadena o componer funciones juntas.</p>
<p className="p">The main benefit of adopting a reactive paradigm is that it raises the level of abstraction of your code, allowing you to focus on specific business logic while forgetting about the arduous boilerplate code associated with setting up asynchronous and event-based programs. Also, this emerging paradigm takes full advantage of FP’s ability to chain or compose functions together.</p>
...
<p className="p">Events come in many flavors: mouse clicks, text field changes, focus changes, handling new HTTP requests, database queries, file writes, and so on. Suppose you need to read and validate a student’s SSN. A typical imperative approach might look like the next listing.</p>
...
<p className="p">This paradigm is enabled through a very important artifact called an observable. Observables let you subscribe to a stream of data that you can process by composing and chaining operations together elegantly. Let’s see it in action and subscribe to a simple input field for a student’s SSN.</p>
...
<p className="p">In this chapter, I briefly touched on topics that will be covered in depth throughout the book as you sink deeper into a functional frame of mind. If you’ve been following all the concepts so far, that’s great, but don’t worry if you missed a few things— that just means you’ve picked up the right book. In traditional OOP, you’re accustomed to programming in the imperative/procedural style; changing this will require you to make a drastic shift in your thought processes as you begin to tackle problems the “functional way.”</p>
...
<li className="litag">	You can reduce the complexity of event-based programs by combining functional with reactive programming.</li>
