...
<p classname="pit">El lenguaje natural tiene ningún paradigma dominante, y tampoco lo hace JavaScript. Los desarrolladores pueden elegir entre una caja de sorpresas de approaches- procedimental, funcional y orientada a objetos, y se mezclan según sea apropiado.</p>
...
<p className="p">Despite its C-like syntax, JavaScript draws lots of inspiration from functional lan- guages like Lisp and Scheme. Their commonalities lie in their support for higher- order functions, closures, array literals, and other features that make JavaScript a superb platform for applying FP techniques. In fact, functions are the main units of work in JavaScript, which means they’re used not only to drive the behavior of your applications, but also to define objects, create modules, and handle events.</p>
<p classname="pit">JavaScript está evolucionando y mejorando activamente. Con el respaldo de la ECMAScript (ES) Stan- dard, su próximo gran lanzamiento, ES6, añade muchas más funciones para el idioma: flecha funciones, constantes, iteradores, promesas y otras características que se adapten a pro- gramación funcional muy bien.</p>
<p className="p">JavaScript is actively evolving and improving. Backed by the ECMAScript (ES) stan- dard, its next major release, ES6, adds many more features to the language: arrow functions, constants, iterators, promises, and other features that suit functional pro- gramming very well.</p>
...
<p className="p">Despite the fact that it has lots of powerful functional features, it’s important to know thatJavaScript is as object-oriented as it is functional. Unfortunately, the latter is rarely seen; most developers use mutable operations, imperative control structures, and instance state changes on objects, which are all virtually eliminated when adopt- ing a functional style. Nevertheless, I feel it’s important to spend some time talking about JavaScript as an object-oriented language first so that you can better appreciate the key differences between the two paradigms. This will allow you to leap into func- tional programming more easily.</p>
...
<p className="p">By applying functional programming, you create an entirely new function, find- StudentsBy, that’s much easier to work with. Keep in mind that this new function works with any objects that relate to Person, as well as any school and country combination.</p>
...
<p className="p">This clearly demonstrates the differences between the two paradigms. Object- oriented design focuses on the nature of data and data relationships, whereas func- tional programming focuses on the operations performed—behavior. Table 2.1 sum- marizes other key differences that are worth noticing as I talk about them in this chapter and others to come.</p>
...
<p classname="pit">He mencionado en el capítulo 1 que el trabajo con funciones puras hace que su código sea más fácil de mantener y razonar acerca. ¿Existe tal cosa como un objeto puro? Un objeto inmutable que contiene funcionalidad inmutable puede considerarse puro. El mismo nivel de razonamiento que se aplica a las funciones del mismo modo que se traduce a objetos simples. Manag- ing estado en JavaScript es crucial en nuestra búsqueda para utilizarlo como un lenguaje funcional. Hay algunas prácticas y patrones que se pueden utilizar para administrar la inmutabilidad, que vamos a visitar en las siguientes secciones, pero encapsulación completa y protección de datos serán un gran peso en su disciplina para hacerlo cumplir.</p>
...
<p classname="pit">Lo que necesita es una política más estricta para la inmutabilidad; y la encapsulación es una buena es- trategia para proteger contra las mutaciones. Para estructuras de objetos simples, una buena alternativa es adoptar el modelo de objetos de valor. Un objeto de valor es uno cuya igualdad no depende de identidad o de referencia, sólo en su valor; una vez declarada, su estado no puede cambiar. Además de los números y cadenas, algunos ejemplos de objetos de valor son tipos como tupla, par, punto, código postal, coordinar, el dinero, la fecha y otros. He aquí un ción ¡Ejecución de Código postal:</p>
...
<p className="p">The returned object effectively behaves like a primitive that has no mutating meth- ods.2 Hence, the toString method, although not a pure function, behaves like one and is a pure string representation of this object. Value objects are lightweight and</p>
...
<p className="p">Object.freeze() can also immobilize inherited attributes. So freezing an instance of Student works exactly the same way and follows the object’s prototype chain protect- ing every inherited Person attribute. But it can’t be used to freeze nested object attri- butes, as shown in figure 2.3.</p>
...
with coordinate.translate() ) are extremely beneficial in your quest to reduce the complexities and intricacies ofJavaScript applications. Next, I discuss the best alterna- tive to centrally managing object changes immutably using a functional approach called lenses.
...
<p className="p">This is, for all practical purposes, similar to a get lastname() method. Nothing impressive so far. What about the setter? Here’s where the magic comes in. Now, call- ing R.set creates and returns a brand-new copy of the object containing the new value and preserves the original instance state (copy-on-write semantics for free!):</p>
...
<p className="p">Lenses are valuable because they give you an unobtrusive mechanism for manipulat- ing objects, even if these are legacy objects or objects outside of your control. Lenses also support nested properties, like the address property of Person:</p>
<pre><code>{`
...
<p classname="pit">funciones de JavaScript tienen dos características importantes que son el pan y ter pero- de su estilo funcional: son de primera clase y de orden superior. Vamos a explorar tanto de estas ideas con detalle a continuación.</p>
<p className="p">JavaScript functions have two important characteristics that are the bread and but- ter of its functional style: they’re first-class and higher-order. We’ll explore both of these ideas in detail next.</p>
...
<p className="p">In addition to being assignable, JavaScript functions like sort() accept other func- tions as arguments and belong to a category called higher-order functions.</p>
...
<p className="p">The following snippet shows that functions can be passed in to other functions. The applyOperation function takes two arguments and applies any operator func- tion to both of them:</p>
<pre><code>{`
...
<p className="p">A noticeable pattern that occurs in languages like JavaScript is that function names can be passive nouns like multiplier, comparator, and action. Because they’re first- class, functions can be assigned to variables and executed at a later time. Let’s refactor printPeople to take full advantage of higher-order functions:</p>
...
<h2>Looking ahead</h2>
<p classname="pit">Quiero hacer una breve pausa mi análisis de núcleo de material JavaScript para dar más detalles sobre el programa en esta sección y combinar algunos conceptos que he mencionado brevemente. Esto es un poco avanzada por ahora, pero pronto aprenderá cómo construir programas de esta manera el uso de técnicas de planificación familiar. Puede crear funciones de apoyo que utilizan lentes que se pueden utilizar para acceder a las propiedades de un objeto:</p>
...
<p className="p">As you can see from these examples, unlike in other programming languages, the this reference is set based on how the function is used (globally, as an object method, as a constructor, and so son) and not by its lexical context (its location in the code). This can lead to code that’s hard to understand, because you need to pay close atten- tion to the context in which a function is executing.</p>
<p classname="pit">He incluido esta sección porque es importante que usted sepa como oper desa- JavaScript; pero, como he indicado varias veces, el uso de esta en el código funcional es rara vez visto (de hecho, ha evitado a toda costa). Es muy utilizada por biblioteca y implement- herramienta ERS para casos especiales que exigen el contexto de flexión lenguaje para realizar hazañas increíbles. Estos implican a menudo la función se aplican métodos y llamada.</p>
...
<p classname="pit">Antes de JavaScript, cierres sólo existía en idiomas FP utilizados en ciertas aplica- ciones específicas. JavaScript es el primero en adoptar en el desarrollo de la corriente principal y signifi- Canti cambiar la forma en que se escribe código. Vamos a revisar el tipo de Código postal:</p>
<p className="p">Prior to JavaScript, closures only existed in FP languages used in certain specific appli- cations. JavaScript is the first to adopt it into mainstream development and signifi- canti change the way in which we write code. Let’s revisit the zipCode type:</p>
<pre><code>{`
function zipCode(code, location) { let _code = code; let _location = location ||	'';
return {
code: function ()	{
...
<p className="p">You may wonder how variables (like additionalVars) declared after a function is declared can also be included as part of its closure. To answer this, you need to under- stand that JavaScript has three forms of scoping: global scope, function scope, and a pseudo-block scope.</p>
...
<p className="p">It’s tempting to use global variables, but they’re shared among all scripts loaded onto the page, which can easily lead to namespace collisions if your JavaScript code isn’t pack- aged into modules. Polluting the global namespace can be problematic because you run the chance of overriding variables and functions declared in different files.</p>
...
<p className="p">isn’t accessible from outside the code block. This can be confusing for developers who are accustomed to that style and are new to JavaScript. Because JavaScript has func- tion scope exclusively, any variables declared in a block are accessible at any point in the function. This can also be a nightmare for JavaScript developers, but there are ways to overcome it. Let’s look at the problem at hand:</p>
...
<p className="p">This is a step in the right direction, and the reason why I prefer using let than var in scope-bounded variables, but manual loops have other shortcomings that we’ll rem- edy in the next chapter. Now that you understand what makes up a function’s closure and its interplay with scope mechanics, let’s turn to some practical uses of closures.</p>
<h2>2.4.4	Practical applications of closures</h2>
<p classname="pit">Los cierres tienen muchas aplicaciones prácticas que son importantes para aplicar cuando imple- Menting programas grandes de JavaScript. Estos no son específicos de la programación funcional, pero sí tomar ventaja mecanismo de la función de ofJavaScript:</p>
<p className="p">Closures have many practical applications that are important to apply when imple- menting large JavaScript programs. These aren’t specific to functional programming, but they do take advantage ofJavaScript’s function mechanism:</p>
...
<p className="p">Using closures, however, it’s possible to emulate this behavior. One example is returning an object, much like zipCode and coordinate in the earlier example. These functions return object literals with methods that have access to any of the outer func- tion’s local variables, but don’t expose these variables, therefore effectively making them private.</p>
...
<p classname="pit">NOTA Como una mejor práctica general, recomiendo el envasado de todo su código fun- cional dentro de módulos bien encapsulado. Puede transferir todos los principios básicos de la programación funcional que ha aprendido en este libro con el nivel de los módulos.</p>
<p className="p">NOTE As a general best practice, I recommend packaging all of your func- tional code inside well-encapsulated modules. You can transfer all the core principles of functional programming you’ve learned in this book to the level of modules.</p>
...
<p className="ind">Index 3 - For a more in-depth explanation of the different types of module patterns, see Ben Cherry’s “JavaScript Mod- ule Pattern: In-Depth,” Adequately Good, March 12, 2010, http://mng.bz/H9hk.</p>
...
<li classname="littag">	Los cierres tienen muchos usos prácticos para la ocultación de información, el módulo de Desa- ción, y pasando el comportamiento parametrizado en funciones de grano grueso a través de múltiples tipos de datos.</li>
<li className="litag">	Closures have many practical uses for information hiding, module develop- ment, and passing parameterized behavior into coarse-grained functions across multiple data types.</li></ul>
