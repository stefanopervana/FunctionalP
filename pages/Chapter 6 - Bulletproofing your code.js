...
Now that you’ve written functional code, how do you prove that it works? In other words, how do you ensure that it meets the specifications laid out by your cus- tomers? The only way is to write code that tests whether the resulting behavior is as
...
designed with that mindset to begin with. In this case, the units are functions that encapsulate your business logic. For example, recali the imperative version of show- Student that you’ve been working on throughout the book. Figure 6.2 shows a good attempt to slice it into its constituent parts.
...
Figure 6.7 shows the result of running the tests with QUnit and Sinon.JS for the test- able parts of showStudent.
...
The fact that functional code is orders of magnitude more testable than imperative code boils down to one principle: referential transparency. The essence of an asser- tion is verifying that referential transparency always holds:
...
By removing the imperative-case clauses, the second statement is much more com- plete. Not only does it express what happens when the student reaches 90 or above, but it also places the restriction that no other numerical range will result in an A. You can derive from the second statement that, at the least, any other computed
...
<li className="litag">	Specifiers—Array describing the type of the input parameters and the specifica- tion with which to generate random datasets.</li>
...
Where did JSC.SSN come from? JSCheck specifiers behave just like functional combi- nators that can be composed to create more-specialized specifiers. This case uses a custom JSC.SSN made from the combination of three JSC.integer specifiers describ- ing the properties of each SSN group, as shown next.
...
produces an 80% total statement-coverage percentage, as shown in the QUnit/Blan- ket output in figure 6.10.
...
Let’s revisit the control flow of the imperative showStudent. To easily delineate the flow, I’ve annotated the statements that translate to nodes in the graph and then gen- erated a flowchart, shown in figure 6.16. Applying the CC formula to this graph with 11 edges, 10 nodes, and 3 exit points yields M = E - N + P = 11 - 10 + 3 = 4.
